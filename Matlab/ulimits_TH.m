...
while (isequal(useall,'n')==1)
    ...
    Xnew
    rangesize = size(Xnew(1:Xsize_1-1,:))
    rangesize = rangesize(1)
    xnewtrs = Xnew(1:Xsize_1-1,:)'
    ynewtrs = Ynew(1:Xsize_1-1,:)'

    markplot = ['.' '*' '+' 'x' 'o' 's' 'd' 'p'];

    for i=1:rangesize
        plot(1000./xnewtrs(:,i),ynewtrs(:,i),markplot(mod(i,8)+1))
        hold on
    end

    % plot(1000./Xnew(1:Xsize_1-1,:)',Ynew(1:Xsize_1-1,:)','--.','LineWidth',0.2)
    xlabel('1000/T')
    ylabel('log_{10}(k)')
    grid on
    hold on
    ...
    plot(1000./tt_Xnew',tt_Ynew','-','Linewidth',1.5)
    title('Arrhenius plot')
    hold on
